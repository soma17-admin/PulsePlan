...
function table(rows) {
  return rows.map((c) => `| ${c.id} | ${c.name} | ${(c.weight * 100).toFixed(0)}% | ${c.auto == null ? "수동 검토" : c.auto + "%"} |`).join("\n");
}

async function main() {
  const rubric = JSON.parse(await readFile(path.join(HERE, "rubric.json"), "utf8"));
  ...
  for (const c of all) console.log(`  ${ICON[c.status]} ${c.id} — ${c.title}: ${c.evidence}`);
  ...
  let judgeOut = null;
  if (wantJudge) {
    console.log(judgeConfigured() ? "▶ LLM-as-judge (Azure Foundry)..." : "▶ LLM 채점 건너뜀(자격증명 없음)");
    judgeOut = await judge(all);
  }

  // 리포트 작성
  const lines = [];
  lines.push(`# TodayMate 제출 전 자가 점검 리포트`);
  lines.push(`생성: ${new Date().toISOString()}\n`);
  lines.push(`## 요약`);
  lines.push(`- 자동 점검: ${all.length}개 / 실패 ${fails.length} / 경고 ${all.filter(c=>c.status==="warn").length}`);
  if (fails.length) lines.push(`- ❌ **차단 항목 있음** — 제출 전 반드시 해결:\n${fails.map(f=>`  - ${f.id}: ${f.evidence}`).join("\n")}`);
  ...
  lines.push(`|------|----|------|------|`);
  for (const c of all) lines.push(`| ${ICON[c.status]} | ${c.id} | ${c.title} | ${c.evidence.replace(/\|/g, "/")} |`);
  lines.push("");
  ...
  if (judgeOut) {
    ...
    if (judgeOut.skipped) {
      ...
      lines.push(`| # | 기준 | 점수 | 근거 |`);
      lines.push(`|---|------|-----:|------|`);
      for (const d of judgeOut.detail) lines.push(`| ${d.id} | ${d.name} | ${d.score} | ${(d.reason||"").replace(/\|/g,"/")} |`);
  ...
  if (fails.length) { console.log(`❌ 차단 항목 ${fails.length}개 — 해결 후 다시 실행하라.`); process.exitCode = 1; }
